...
log_group_name_api = os.environ.get("LOG_GROUP_API", "/aws/lambda/bedrock_invoke_model")
s3_bucket = os.environ.get("S3_BUCKET", "hackathon-s3-bucket-29-02-2024")
...
def process_event(event):
    try:
        if "date" in event:
            ...
            date = datetime.datetime.now(pytz.UTC) - datetime.timedelta(hours=1)
            # get me the hour of date and store it as a string variable
        ...
        if len(df_bedrock_cost_tracking) > 0:
            ...
            df_bedrock_cost_tracking[["input_tokens", "output_tokens", "input_cost", "output_cost",
                                      "invocations"]] = df_bedrock_cost_tracking.apply(
            ...
            file_name = f"{date}/{date_hour}.csv"
